...
import { convertToHexFormat, formatter } from '../../helper';
...
export const ProductDetail: FC<Props> = ({ items, item }) => {
  ...
  const youMayAlsoLIke = items.filter(anyItem =>
    anyItem.name.includes(selectedName?.toString() || ''),
  );
            ...
            <div className="productDetail__top top">
              <section className="top__photo">
                ...
                <div className="photo__current">
                  <img
                    src={`${BASE_URL_PHOTO}/${image || item.images[0]}`}
                    alt="Current"
                  />
                </div>
              </section>
                ...
                <div className="parameters__block">
                  ...
                  <ul className="parameters__avaible-colors">
                    {colorsAvailable.map(currentColor => (
                      <li
                        key={currentColor}
                        className={cn('parameters__avaible-color', {
                          active__color: color === currentColor,
                        })}
                        style={{
                          backgroundColor: convertToHexFormat(
                            currentColor.replaceAll(' ', ''),
                          ),
                        }}
                      >
                        <Link
                  ...
                  <ul className="parameters__capacity">
                    {capacityAvailable.map(cpct => (
                      <li key={cpct}>
                        <Link
                          to={getCapacity(cpct)}
                          className={cn('parameters__cpct-link', {
                ...
                <div className="about__info">
                  {description.map(({ text, title }) => (
                    <div className="about__info-item" key={title}>
                      ...
                      <span className="about__info-text">{text[0]}</span>
                      <span className="about__info-text about__info-text--one">
                        {text[1]}
                      </span>
                    </div>
                  ))}
