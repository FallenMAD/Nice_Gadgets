...
export const ProductsList: FC<Props> = ({ products }) => {
  ...
  const lastItem = Math.min(page * perPage, sortedProducts.length);

  const currentItems = sortedProducts.slice(firstItem, lastItem);

  const onPageChange = (value: number) => {
    setSearchWith(searchParams, { page: value || null }, setSearchParams);
  };
  ...
  const handleSortChange = (value: string) => {
    ...
    setSearchWith(searchParams, { sort: sortValue || null }, setSearchParams);
  };
